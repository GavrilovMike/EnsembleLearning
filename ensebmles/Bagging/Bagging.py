...
model = BaggingClassifier(base_estimator=SVC(),
                        n_estimators=8, random_state=0).fit(qtable_X, qtable_decisioned_X)
...
for i in range (76):
        for j in range (7):
            if (y_pred[i] == 0):
                Q_table_bagging[i][0] = 0

            if (y_pred[i] == 1):
                Q_table_bagging[i][1] = 111

            if (y_pred[i] == 2):
                Q_table_bagging[i][2] = 222

            if (y_pred[i] == 3):
                Q_table_bagging[i][3] = 333

            if (y_pred[i] == 4):
                Q_table_bagging[i][4] = 444

            if (y_pred[i] == 5):
                Q_table_bagging[i][5] = 555

            if (y_pred[i] == 6):
                Q_table_bagging[i][6] = 666

            if (y_pred[i] == 7):
                Q_table_bagging[i][7] = 777


for i in range (76):
        for j in range (7):
            if (Q_table_bagging[i][j] == 0):
...
print('Bagging QTable: \n', Q_table_bagging, '\n')
print(Q_table_bagging.shape)


with open("Bagging_QTable.pkl", 'wb') as f3:
    pickle.dump(Q_table_bagging, f3)